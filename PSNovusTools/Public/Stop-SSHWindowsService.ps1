<#
.SYNOPSIS
    Stops a Windows service on a remote machine using SSH.
.DESCRIPTION
    This function uses the Posh-SSH module to connect to a remote Windows machine via SSH and Stop a specified service.
#>
function Stop-SSHWindowsService {
    ...
    process {
        ...
        try {
            ...
            $sshWindowsService = Stop-SSHSessionWindowsService -session $session -remoteServiceName $remoteServiceName
      
          
        }
        catch {
            Write-Error "Failed to Stop service '$remoteServiceName' on '$remoteServer'. Error: $_"
            $result = $false
        }
        finally {
