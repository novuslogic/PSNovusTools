...
function Invoke-SSHItemBash {
    ...
    param(
        ...
        [Parameter(Mandatory = $true, HelpMessage = "Enter the remote SSH Bash command.")]
        [ValidateNotNullOrEmpty()]
        [string]$sshBashCommand
...
try {
   ...
   $sshresult = Invoke-SSHSessionItemBash -session $session -plainTextPassword $plainTextPassword -sshBashCommand $sshBashCommand
